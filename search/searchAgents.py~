...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.cornersUnTouched = self.corners
        self.startingState = (self.startingPosition, self.cornersUnTouched)
    def getStartState(self):
        """
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                restCorners = []
                if(state[0] in self.cornersUnTouched):
                    for corner in self.cornersUnTouched:
                        if(corner != state[0]):
                            restCorners.append(corner)
                else:
                    restCorners = list(self.cornersUnTouched)

                nextState = ((nextx, nexty),tuple(restCorners))  
...
def cornersHeuristic(state, problem):
    ...
    """
    corners = problem.corners # are the corner coordinates
    walls = problem.walls # These are the walls of the maze, as a Grid (game.py)
    cornerList = list(corners)
    #print "first :",cornerList
    xy1 = state[0] 
    corners2 = list(corners) 
    minVal = 100000 
    for i in range(len(corners2)): 
        xy2 = corners2[i] 
        heur = ( (xy1[0] - xy2[0]) ** 2 + (xy1[1] - xy2[1]) ** 2 ) ** 0.5 
        #heur = abs((xy1[0] - xy2[0]) + abs(xy1[1] - xy2[1])) 
        minVal = min(minVal, heur) #print minVal 
    return minVal
    pacmanPos = state[0]
    pathCost = 0
    if(len(cornerList) == 0): return 0
        ...
        while len(cornerList) > 0 :
            ...
            for corner in cornerList:
                distance.append(abs(pacmanPos[0] - corner[0]) + abs(pacmanPos[1] - corner[1]))
                #print distance
            minDistance = min(distance)
            pathCost += minDistance
            
            pacmanPos = cornerList[distance.index(minDistance)]
            cornerList.remove(pacmanPos)
...
def foodHeuristic(state, problem):
    ...
    #below code fails admissability
